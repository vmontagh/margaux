...
public abstract class ServerSocketListener
	...
	protected final static Logger logger = Logger
			.getLogger(ServerSocketListener.class.getName() + "--"
	...
	public ServerSocketListener(final String hostName, final int hostPort,
	...
	public ServerSocketListener(final InetSocketAddress hostAddress,
	...
	protected void changeHostAndRemoteAddress(final InetSocketAddress hostAddress,
			final InetSocketAddress remoteAddress) {
		synchronized (lock) {
			if (!running) {
				this.hostAddress = hostAddress;
		...
		try {
			...
			while (!Thread.currentThread().isInterrupted()) {
				try {
					...
					if (Configuration.IsInDeubbungMode)
						logger.info(Utils.threadName() + " a message is received: "
								+ hostAddress.getPort());
