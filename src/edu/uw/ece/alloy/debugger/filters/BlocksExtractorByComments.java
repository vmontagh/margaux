...
/**
 * Given and alloy program, one can annotate the expressions need to be
 * analyzed. For example:
 * 
 * sig A{r:A} fact{ /*@begin-analyze*\/ some r /*@end-analyze*\/ }
 * 
...
public abstract class BlocksExtractorByComments {
	...
	public List<Pos> findAllPairs() {
		...
		for (String line : Utils.readFileLinesWithEmptyLines(path)) {
			...
			while (foundStart >= 0) {
				allPoses
						.add(new Pos(START, foundStart + START.length() + 1, lineNumber));
		...
		for (Pos pos : allPoses) {
			...
			if (pos.filename.equals(END)) {
				Pos start = stack.pop();
				result.add(new Pos(new File(path).getAbsolutePath(), start.x, start.y,
						pos.x, pos.y));
			} else {
				stack.push(pos);
			}
	...
	public List<String> getAllBlocks() {
		return findAllPairs().stream().map(p -> Utils.readSnippet(p))
	...
	public static class ExtractExpression extends BlocksExtractorByComments {
		...
		public List<Expr> getAllExpressions() throws Err {
			...
			final CompModule compModule = CompUtil
					.parseEverything_fromFile(A4Reporter.NOP, null, path);
		...
		/**
		 * Given an immutable path, getAllExpressionsAndFields extracts all snippets
		 * and returns all the fields are directly or indirectly references by the
		 * snippets.
		 * 
		 * @return
		 * @throws Err
		 */
		public Map<String, List<Sig.Field>> getAllExpressionsAndFields()
				throws Err {
