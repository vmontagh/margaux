...
public class OnBorderInstanceFinder extends ServerSocketListener {
	...
	protected final static Logger logger = Logger
			.getLogger(OnBorderInstanceFinder.class.getName() + "--"
					+ Thread.currentThread().getName());

	private final static String ProcessLoggerConfig = Configuration
			.getProp("process_logger_config");
	private final static int SubMemory = Integer
			.parseInt(Configuration.getProp("sub_memory"));
	private final static int SubStack = Integer
			.parseInt(Configuration.getProp("sub_stak"));
	private final static File tmpDirectoryRoot = new File(
	...
	public OnBorderInstanceFinder(final InetSocketAddress localPort,
			final InetSocketAddress remotePort, final String filePathArgs,
			final String propertiesFile) {

		// The local port is the port we are listening to (We are listening to our
		// own port)
		// For now, set the remote port to be null, it will be set after the process
		// has been created
		...
		this.tmpDirectory = new File(tmpDirectoryRoot,
				String.valueOf(PID.getPort()));
		setUpFolders();
	}
	...
	public void execute() throws UnknownHostException {
		...
		// This process must stop before the interval is finished or it will be
		// terminated.
		...
		this.watchdog.startTimer(new Runnable() {
			...
			public void run() {
				...
				logger.info(Utils.threadName()
						+ "Watchdog timer terminated. Running timer callback");
				...
				if (finalResult != null) {
					...
					try {
						...
						logger.log(Level.SEVERE,
								Utils.threadName()
										+ "Interrupted while sending final result to remote. Remote Address: "
										+ remotePort,
								e);
					}
	...
	protected void processCommand(final RemoteCommand command) {
		...
		System.out.println("Commands is received in "
				+ OnBorderInstanceFinder.class.getSimpleName() + ": " + command);
	...
	protected void onStartingListening() throws InterruptedException {
		if (Configuration.IsInDeubbungMode)
			logger.info(Utils.threadName() + " The process is started on port : "
					+ this.getHostAddress().getPort());
	...
	private synchronized HolaProcess createProcess(
			final InetSocketAddress address) {

		Process sub = null;
		final String java = "java";
		final String jarPath = new java.io.File(HolaRunner.class
		...
		try {
			...
			String[] commands = { java, "-jar", jarPath, "" + this.PID.getPort(),
					"" + this.PID.getAddress().getHostAddress(), "" + address.getPort(),
					"" + address.getAddress().getHostAddress(), "" + this.filePathArgs,
					"" + this.propertiesFile };
	...
	private void setUpFolders() {
		...
		if (this.tmpDirectory.exists()) {
			try {
				if (Configuration.IsInDeubbungMode)
					logger.info(Utils.threadName() + " exists and has to be recreated."
							+ tmpDirectory.getCanonicalPath());
				Utils.deleteRecursivly(this.tmpDirectory);
			} catch (IOException e) {
				logger.log(Level.SEVERE,
						Utils.threadName() + "Unable to delete the previous files.", e);
			}
		}
	...
	public static void main(String[] args) {
		...
		if (Configuration.IsInDeubbungMode)
			logger.info(Utils.threadName() + "The '"
					+ OnBorderInstanceFinder.class.getSimpleName()
					+ "' process has started.");
		...
		try {
			...
			if (Configuration.IsInDeubbungMode)
				logger
						.info(Utils.threadName() + "The port is assigned to this process: "
								+ localPort + " and the IP is: " + localIP);

			remotePort = Integer.parseInt(args[2]);
			remoteIP = InetAddress.getByName(args[3]);

			if (Configuration.IsInDeubbungMode)
				logger.info(Utils.threadName() + "The remote port is: " + remotePort
						+ " and the IP is: " + remoteIP);
			...
			logger.log(Level.SEVERE,
					Utils.threadName() + "The passed port is not acceptable: ",
					nfe.getMessage());
			throw new RuntimeException("The port number is not an integer: " + nfe);
		} catch (UnknownHostException uhe) {
			logger.log(Level.SEVERE,
					Utils.threadName() + "The passed IP is not acceptable: ",
					uhe.getMessage());
		...
		OnBorderInstanceFinder finder = new OnBorderInstanceFinder(localSocket,
				remoteSocket, filePathArgs, propertiesFile);

		try {
			finder.execute();
		} catch (UnknownHostException e) {
			logger.log(Level.SEVERE, Utils.threadName() + "Unknown Host exception",
					e);
			e.printStackTrace();
		}
		...
		// Thread.sleep(100000);
		// } catch (InterruptedException e) {
		// logger.log(Level.SEVERE, Utils.threadName() + "Main loop is interrupted
		// ", e);
		// break;
		// }
		//
		// if(Configuration.IsInDeubbungMode) logger.info(Utils.threadName() + "Main
		// is alive.... ");
		// Thread.yield();
		// }

		logger.info(Utils.threadName()
				+ OnBorderInstanceFinder.class.getSimpleName() + " exiting.");
	}
}
