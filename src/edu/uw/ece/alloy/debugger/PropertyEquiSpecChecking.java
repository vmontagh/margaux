...
public class PropertyEquiSpecChecking extends PropertyCheckingSource {
	...
	final String assertionFFNameFormat = new String(
			"%s" + SEPARATOR + "_F__F_" + SEPARATOR + "%s" + SEPARATOR + "%s");
	// Spec => relation
	...
	public PropertyEquiSpecChecking(File sourceFile_, String property_,
		...
		this.assertionName = String.format(assertionFFNameFormat, commandHeader,
				sanitizedPropertyName, sanitizedFieldName);
		this.assertionBody = String.format(propertyCheckingFormatForward,
				assertionName, formula, ExprBinary.Op.IFF.toString(), property,
				commandScope);
