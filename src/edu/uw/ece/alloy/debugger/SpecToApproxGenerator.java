...
public class SpecToApproxGenerator extends PropertyCheckingSource {
	...
	public String toAlloy() {
		...
		/*
		 * for(Operator operator: Operator.values()){ if(
		 * !commandHeaderToProperties.containsKey(operator) ) continue; Map<String,
		 * Set<String>> map = commandHeaderToProperties.get(operator); for(String
		 * commandHeader: map.keySet()){ final String predName =
		 * "approx_"+operator+"_"+commandHeader; final String body =
		 * map.get(commandHeader).stream().collect(Collectors.joining(" \n "));
		 * newAlloySpec.append(String.format("pred %s{%n %s %n}%n", predName,
		 * body)).append("\n"); } }
		 */
	 ...
	 * @return
	 */
	public static Set<Pair<PropertyRelation, PropertyRelation>> findProps(
			final Operator operatorI, final Operator operatorJ,
			final BiPredicate<Property, Property> checker) {
				...
				for (final Statement stmtq : stmts) {
					...
					for (Property p : Ps) {
						for (Property q : Qs) {
							...
							if (checker.test(p, q)) {
								final PropertyRelation prP = new PropertyRelation(p, stmtp,
										fld);
								final PropertyRelation prQ = new PropertyRelation(q, stmtq,
										fld);
								final Pair<PropertyRelation, PropertyRelation> pair = new Pair<>(
										prP, prQ);
								result.add(pair);
							}
						}
	...
	public static Set<Pair<PropertyRelation, PropertyRelation>> findI_a() {
		return findProps(Operator.Consistent, Operator.Inconsistent,
				PrecomputedProperties.INSTANCE.isEqual);
	}

	public static Set<Pair<PropertyRelation, PropertyRelation>> findI_b() {
		return findProps(Operator.Consistent, Operator.Inconsistent,
				PrecomputedProperties.INSTANCE.isImplied);
	}

	public static Set<Pair<PropertyRelation, PropertyRelation>> findI_c() {
		return findProps(Operator.Consistent, Operator.Inconsistent,
				PrecomputedProperties.INSTANCE.isConsistent);
	}

	public static Set<Pair<PropertyRelation, PropertyRelation>> findI_d() {
		return findProps(Operator.Consistent, Operator.Inconsistent,
				PrecomputedProperties.INSTANCE.isRevImplied);
	}

	public static Set<Pair<PropertyRelation, PropertyRelation>> findI_e() {
		return findProps(Operator.Consistent, Operator.Inconsistent,
				PrecomputedProperties.INSTANCE.isInconsistent);
	}

	public static Set<Pair<PropertyRelation, PropertyRelation>> findII_a() {
		return findProps(Operator.CompleteHas, Operator.Inconsistent,
				PrecomputedProperties.INSTANCE.isEqual);
	}
	...
	public static Set<Pair<PropertyRelation, PropertyRelation>> findIII_a() {
		return findProps(Operator.CompleteHas, Operator.Consistent,
				PrecomputedProperties.INSTANCE.isEqual);
	}
	...
	public static Set<Pair<PropertyRelation, PropertyRelation>> findIV_a() {
		return findProps(Operator.Consistent, Operator.Consistent,
				PrecomputedProperties.INSTANCE.isEqual);
	}
	...
	public static Set<Pair<PropertyRelation, PropertyRelation>> findV_a() {
		return findProps(Operator.CompleteHas, Operator.CompleteHas,
				PrecomputedProperties.INSTANCE.isEqual);
	}
	...
	public static Set<Pair<PropertyRelation, PropertyRelation>> findVI_a() {
		return findProps(Operator.Inconsistent, Operator.Inconsistent,
				PrecomputedProperties.INSTANCE.isEqual);
	}
	...
	static {
		/*
		 * included.add(new Statement("A")); included.add(new Statement("B"));
		 * included.add(new Statement("C")); included.add(new Statement("CF1"));
		 * included.add(new Statement("CF2")); included.add(new Statement("CF3"));
		 */
	...
	public static Set<Pair<PropertyRelation, PropertyRelation>> filterMap(
			final Set<Pair<PropertyRelation, PropertyRelation>> map) {
		return map.stream()
	...
	public static void printMap(
		...
		filterMap(map).stream()
				...
				// .filter(a-> (included.contains(a.a.stmt) &&
				// included.contains(a.b.stmt)))
				// .forEach(a->System.out.printf("%s %s %s %s %n", a.a.value,
				// included.contains(a.a) , map.get(a).a.value,
				// included.contains(map.get(a).a)) );
				// .filter(a-> included.contains(a.a.value) &&
				// included.contains(map.get(a).b.a.value))
	...
	/**
	 * In the given set we have ([a,b],[p,q]) in state statement S over the field
	 * r and we know that a=>p, then the output is ([a,b], [a,q])
	 * 
	 * @param set
	 * @return
	 */
	public static Set<Pair<PropertyRelation, PropertyRelation>> filterbyImplicationRightside(
			final Set<Pair<PropertyRelation, PropertyRelation>> set) {

		final Set<Pair<PropertyRelation, PropertyRelation>> result = set.stream()
				.filter(
						a -> (included.contains(a.a.stmt) && included.contains(a.b.stmt)))
				.collect(Collectors.toSet());
		...
		do {
			...
			initResult = new HashSet<Pair<PropertyRelation, PropertyRelation>>(
					result);

			// needed to find the concrete parents.
			final Set<Property> propsFrom = result.stream().map(a -> a.a.prop)
					.collect(Collectors.toSet());
			// needed to find the concrete parents.
			final Set<PropertyRelation> rightPropRelations = result.stream()
					.map(a -> a.a).collect(Collectors.toSet());
			// for concurrency modification.
			final Set<Pair<PropertyRelation, PropertyRelation>> pairs = new HashSet<Pair<PropertyRelation, PropertyRelation>>(
					result);
			...
			for (final Pair<PropertyRelation, PropertyRelation> pair : pairs) {
				...
				final List<Property> parentsPFrom = PrecomputedProperties.INSTANCE
						.getImplicationParents(pFrom, propsFrom);
				boolean shrunk = false;
				for (Property parent : parentsPFrom) {
					// There is a pair containing the parent?
					PropertyRelation tmpParent = new PropertyRelation(parent, pair.a.stmt,
							pair.a.fld);

					if (rightPropRelations.contains(tmpParent)) {
						// pair is [p,q], a(i.e. tmpParent) is the parent p(i.e. pair.a),
						// i.e. a=>p OR pair.a => tmpParent
						// remove the pair, i.e. [p,q]
						result.remove(pair);
						// add [a,q]
						result.add(new Pair<PropertyRelation, PropertyRelation>(tmpParent,
								pair.b));
						shrunk = true;
						break;
	...
	/**
	 * In the given set we have ([a,b],[p,q]) in state statement S over the field
	 * r and we know that b=>q, then the output is ([a,q], [p,q])
	 * 
	 * @param set
	 * @return
	 */
	public static Set<Pair<PropertyRelation, PropertyRelation>> filterbyImplicationLeftside(
			final Set<Pair<PropertyRelation, PropertyRelation>> set) {
		...
		do {
			...
			// needed to find the concrete parents.
			final Set<Property> propsTo = result.stream().map(a -> a.b.prop)
					.collect(Collectors.toSet());
			// needed to find the concrete parents.
			final Set<PropertyRelation> leftPropRelations = result.stream()
					.map(a -> a.b).collect(Collectors.toSet());
			// for concurrency modification.
			final Set<Pair<PropertyRelation, PropertyRelation>> pairs = new HashSet<Pair<PropertyRelation, PropertyRelation>>(
					result);
			...
			for (final Pair<PropertyRelation, PropertyRelation> pair : pairs) {
				...
				// find all parents of right of the the current pair
				final List<Property> childrenPTo = PrecomputedProperties.INSTANCE
						.getImplicationChildren(pTo, propsTo);
				boolean shrunk = false;
				for (Property child : childrenPTo) {
					// There is a pair containing the child?
					PropertyRelation tmpChild = new PropertyRelation(child, pair.b.stmt,
							pair.b.fld);

					if (leftPropRelations.contains(tmpChild)) {
						// pair is [a,b], q(i.e. tmpChild) is the child b(i.e. pair.b), i.e.
						// b=>q OR pair.b => tmpChild
						// remove the pair, i.e. [p,q]
						result.remove(pair);
						// add [a,q]
						result.add(
								new Pair<PropertyRelation, PropertyRelation>(pair.a, tmpChild));
						shrunk = true;
						break;
