...
public class PropertiesDecomposer {
	...
	public static void main(String[] args) throws Err, IOException {
		...
		A4Reporter rep = new A4Reporter() {
			...
			public void translate(String solver, int bitwidth, int maxseq,
				...
				lastTime = System.currentTimeMillis();
				/*
				 * System.out.println("translate->Solver="+solver+" Bitwidth="+bitwidth+
				 * " MaxSeq="+maxseq + (skolemDepth==0?"":" SkolemDepth="+skolemDepth) +
		...
		/*
		 * Module world1 = CompUtil.parseEverything_fromFile(rep, null,
		 * "models/debugger/tmp-propDeadlock_irreflexiveStateholds.als"); A4Options
		 * options2 = new A4Options(); options2.solver =
		 * A4Options.SatSolver.MiniSatJNI; Command command2 =
		 * world1.getAllCommands().get(0); A4Solution ans2 =
		 * TranslateAlloyToKodkod.execute_command(rep, world1.getAllReachableSigs(),
		 * command2, options2); System.out.println(ans2.satisfiable());
		 * System.exit(-10);
		 */
		...
		String cmdName = args[2];
		{
			...
			for (Sig sig : world.getAllSigs()) {
				for (Sig.Field field : sig.getFields()) {
					...
					if (field.type().arity() == 3)
						fields
								.add(field.sig.label.replace("this/", "") + "." + field.label);
				}
			}
			...
			for (Command command : world.getAllCommands()) {
				if (command.label.equals(cmdName)) {
					scope = command.toString()
							.substring(command.toString().indexOf("for"));
				}
			...
			while (scnr.hasNextLine()) {
				...
				for (String field : fields) {
					...
					final String propOutputFile = "models/debugger/tmp-prop" + newFileName
							+ ".als";
					// System.out.println(propOutputFile);
					Util.writeAll(propOutputFile,
							"open util/relation\n open general_properties\n" + source + "\n"
									+ newCommand);
					Module propWorld = CompUtil.parseEverything_fromFile(rep, null,
							propOutputFile);
					A4Options options = new A4Options();
					options.solver = A4Options.SatSolver.MiniSatJNI;
					Command command = propWorld.getAllCommands().get(0);
					A4Solution ans = TranslateAlloyToKodkod.execute_command(rep,
							propWorld.getAllReachableSigs(), command, options);
					result
							.append(prop.substring(prop.indexOf("=> ") + "=> ".length(),
									prop.indexOf("[?")))
							.append("\t").append(field).append("\t")
							.append(!ans.satisfiable()).append("\n");
				}
			}
	...
	private static String exrtactPos(Pos pos, String fileName)
