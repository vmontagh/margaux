...
public class ProcessIt extends RemoteCommand {
	...
	final static Logger logger = Logger.getLogger(
			ProcessIt.class.getName() + "--" + Thread.currentThread().getName());
		...
		try {
			if (Configuration.IsInDeubbungMode)
				logger.fine("[" + Thread.currentThread().getName() + "] "
						+ "Received a message: " + param);

			AlloyProcessingParam param = this.param.prepareToUse();
			// tmpDirectory is sent at the client side.
			param = param
					.changeTmpLocalDirectory(AlloyProcessRunner.getInstance().tmpDirectory);
			param = param.dumpAll();
			// System.out.println(param.srcPath());
			executer.process(param);
			if (Configuration.IsInDeubbungMode)
				logger.fine("[" + Thread.currentThread().getName() + "] "
						+ "Prepared and queued: " + param);
		} catch (Exception e) {
			logger
					.log(Level.SEVERE,
							"[" + Thread.currentThread().getName() + "] "
									+ "Failed on prepare or execute the message: " + this.param,
							e);
			e.printStackTrace();
		}
	...
	public String toString() {
		 ...
		 * "Failed on decompressing the message: "+ param, e); e.printStackTrace();
		 * }
		 */
