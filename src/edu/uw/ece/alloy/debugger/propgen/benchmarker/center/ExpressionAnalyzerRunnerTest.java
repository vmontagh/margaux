...
public class ExpressionAnalyzerRunnerTest {
	...
	public void testStartAlloyReadyGetReadyness() {
		ExpressionAnalyzerRunner runner = new ExpressionAnalyzerRunner(runnerHost, testingHost);
		runner.initiate();
	...
	@Test
	public void testStartAlloySetupGetLiveness() {
	...
	public void testExecution(String patterns) throws Err {
		...
		LazyFile relationalPropModuleOriginal = new LazyFile(tmpLocalDirectory, "relational_properties_tagged.als");
		Util.writeAll(relationalPropModuleOriginal.getAbsolutePath(), patterns);

		LazyFile temporalPropModuleOriginal = new LazyFile(tmpLocalDirectory, "temporal_properties_tagged.als");
		...
		LazyFile relationalLib = new LazyFile(tmpLocalDirectory, "relational_lib.als");
		...
		testingInterface.MessageReceived.addListener(new MessageEventListener<MessageReceivedEventArgs>() {
			...
			public void actionOn(ReadyMessage readyMessage, MessageReceivedEventArgs messageArgs) {
				...
				// send a setup message
				testingInterface.sendMessage(new PatternSetupMessage(testingInterface.getHostProcess(),
						Arrays.asList(relationalPropModuleOriginal.load(), temporalPropModuleOriginal.load(), relationalLib.load())));
				++readynessReceived.val;
				synchronized (readynessReceived) {
					readynessReceived.notify();
				}
		...
		// toBeAnalyzedCode
		LazyFile toBeAnalyzedCode = new LazyFile(tmpLocalDirectory, "toBeAnalyzedCode.als");
		Util.writeAll(toBeAnalyzedCode.getAbsolutePath(), "sig A{r: A}\n pred p[]{ no ~r & r} run{}");
		...
		PatternProcessingParam param = new PatternProcessingParam(0, tmpLocalDirectory, UUID.randomUUID(),
				Long.MAX_VALUE, "r", IfPropertyToAlloyCode.EMPTY_CONVERTOR, "p", " for 5", files);

		PatternRequestMessage request = new PatternRequestMessage(testingInterface.getHostProcess(), param);
