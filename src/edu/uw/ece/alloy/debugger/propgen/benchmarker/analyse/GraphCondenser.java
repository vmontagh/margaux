...
import edu.uw.ece.alloy.util.graph.DijkstraSP;
import edu.uw.ece.alloy.util.graph.DirectedEdge;
import edu.uw.ece.alloy.util.graph.EdgeWeightedDigraph;
...
public class GraphCondenser {
	...
	/*
	 * an item in the array is either -1, itself, or other value. -1 means the
	 * node is deleted. itself means it is not replaced by other node other value
	 * means the node has been replaced by other node. e.g if a<=>B then b is
	 * replaced by a
	 */
	...
	/*
	 * Each index in implicationClasses contains a list of other nodes having iff
	 * to it. For example, implicationClasses[0]={1,2} means 0<=>1, 0<=>2, and
	 * 1<=>2 It is expected to see implicationClasses[1]={0,2} and
	 * implicationClasses[2]={1,0}
	 */
	final private TreeSet<Integer> implicationClasses[] = new TreeSet[nodeCount];
	/*
	 * After processing implicationClasses, the iff-class reps are stored in
	 * confirmingReplacedNode which has to be equal to replacedNode.
	 */
	...
	public GraphCondenser() {
		Stream.range(0, nodeCount).forEach(i -> {
			replacedNode[i] = i;
	...
	public final String makeHTMLLegends() {
		...
		result.append("\t")
				...
				.append("\n");
		result.append("\t\t")
				.append("<TR><TD COLSPAN=\"2\"><B>Legend</B></TD></TR>").append("\n");
		...
		Stream.range(1, nodeCount).forEach(
				...
				i -> {
					...
					if (vacuities.contains(i)) {
						...
						fontEnd = "</FONT>";
					} else if (replacedNode[i] != i) {
						key = key + "&rarr;" + replacedNode[i];
						fontStart = "<FONT COLOR=\"green\">";
						fontEnd = "</FONT>";
					}
					result.append("\t\t").append("<TR><TD>").append(fontStart).append(key)
							.append(fontEnd).append("</TD>");
					result.append("<TD>").append(fontStart).append(value).append(fontEnd)
							.append("</TD></TR>").append("\n");
				}
	...
	public final void process() {
		...
		/*
		 * traversing the nodes from 0 to nodeCount and find loop backs, i.e. a->b
		 * and b->a. In this case b is removed and all its appearances are replaces
		 * by a. replacedNode[b] = a.
		 */
		Stream.range(1, nodeCount).forEach(from -> {
			if (replacedNode[from] == -1) {
				sparceMatrixImplication[from].clear();
				;
			} else {
				sparceMatrixImplication[from].stream().forEach(to -> {
	...
	/**
	 * Given the analyzed iff statements, stored in iffCSVPath, the reps-classes
	 * computed in confirmingReplacedNode and replacedNode have to be equal.
	 */
			...
			for (Integer j : iffSet) {
				if (!implicationClasses[j].containsAll(iffSet)) {
					...
					System.err.printf("Invalid total order state %s-%s: \n\t%s \n\t%s%n",
							i, j, iffSet, implicationClasses[j]);
	...
	public final void condese() {
		...
		// TODO
		if (!confirmProcessValidity())
			throw new RuntimeException();
		for (int node = 1; node < implicationClasses.length; ++node) {
			if (vacuities.contains(node)) {
				replacedNode[node] = -1;
			} else {
				replacedNode[node] = implicationClasses[node].isEmpty() ? node
						: implicationClasses[node].first();
			}
		}
	...
	/**
	 * Precondition: replacedNode contains the information about vacuities, i.e.
	 * nodes are vacuite are -1. Node that are grouped have the rep. i.e. in a
	 * group {1,2,3}, {1} is the rep, and the replacedNode would be [1,1,1].
	 * 
	 * Postcondition: all condensed nodes and vacuite are removed from the the
	 * matrix. The output is meaningful by both replacedNode and sparceMatrix.
	 */
	...
	public final String generateEdges(final String edgeAttribute,
			final Set<Integer> sparceMatrix[]) {
		final StringBuilder dot = new StringBuilder();
		IntStream.range(1, nodeCount).filter(i -> replacedNode[i] == i)
				.forEach(i -> sparceMatrix[i].stream()
						.forEach(j -> dot.append("\t").append(i).append("->").append(j)
								.append(edgeAttribute).append(";\n")));
		return (dot.toString());
	}
	...
	public final String generateDot(final String legend, final String edges) {
		...
		// IntStream.range(1,
		// nodeCount).filter(i->replacedNode[i]==i).forEach(i->System.out.printf("%s->
		// \t%s%n", i, sparceMatrix[i]));
		final StringBuilder dot = new StringBuilder();
	...
	public final String generateDotImplication() {
		...
		return generateDot(makeHTMLLegends(),
				generateEdges("", sparceMatrixImplication));
	}
	...
	public final String generateDotInconsistencies() {
		...
		System.out.println(Arrays.asList(sparceMatrixInconsistency));
		return generateDot(makeHTMLLegends(),
				generateEdges(" [dir=none]", sparceMatrixInconsistency));
	}
	...
	public final String generateDotImplicationPlusInconsistencies() {
		...
		condeseByGrouping(sparceMatrixInconsistency);
		return generateDot(
				/* makeHTMLLegends() */"{ rank = sink;\nLegend [shape=none, margin=0, label=<Blue line: Implications\nRed Lines: Inconsistencies\nBlack Line: Nodes circles>]}",
				generateEdges("[dir=none color=\"red\"]", sparceMatrixInconsistency)
						+ generateEdges(" [color=\"blue\"]", sparceMatrixImplication));
	}
	...
	public List<Integer> findSources(final Set<Integer> sparceMatrix[]) {
		...
		Stream.range(1, nodeCount).forEach(i -> {
	...
	public List<String> decodeToLegends(final List<Integer> nodes) {
		return nodes.stream().map(node -> legends[node])
		...
		for (int source : findSources(sparceMatrix)) {
			for (int sink : findSinks(sparceMatrix))
				System.out.printf("%s \t\t %s(%s) ~> %s(%s)\n",
						(new DijkstraSP(graph, coder.get(source))).distTo(coder.get(sink)),
						legends[source], source, legends[sink], sink);
		}
	...
	public static void main(String[] args) throws Err {
		...
		gc.condese();
		Util.writeAll(String.format(gc.dotOutput, "incosnt"),
				gc.generateDotInconsistencies());
		Util.writeAll(String.format(gc.dotOutput, "imply"),
				gc.generateDotImplication());
		Util.writeAll(String.format(gc.dotOutput, "inconst.imply"),
				gc.generateDotImplicationPlusInconsistencies());
