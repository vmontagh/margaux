...
public class AlloyProcessingParamTester {
	...
	final static Logger logger = Logger.getLogger(AlloyProcessingParamTester.class.getName()+"--"+Thread.currentThread().getName());
	...
	@Test
	public void testAlloyProcessingParam_DumpFile_reWriteEnable() throws Exception {
		
		//A file exists
		//The content has to be have newline at the very last. Utils.writeAll append a newline to the end of 
		//string does not have newline at the last character 
		
		PropertyToAlloyCode vacPropertyToAlloyCode = 
				VacPropertyToAlloyCode.EMPTY_CONVERTOR.createIt(
						predBodyA, predBodyB, 
						predCallA, predCallB, 
						predNameA, predNameB, 
		...
		final Field reWrite = AlloyProcessingParam.EMPTY_PARAM.getClass().getDeclaredField("reWrite");
	...
	@Test
	public void testAlloyProcessingParam_DumpFile_reWriteDisable() throws Exception {
	...
	@Test
	public void testAlloyProcessingParamLazyCompressed_prepareToUse() throws Exception {
		...
		assertNotEquals(srcPath.getAbsolutePath() , aParam2.srcPath().getAbsolutePath()  );
		assertNotEquals(destPath.getAbsolutePath(), aParam2.destPath().getAbsolutePath() );
		assertEquals   (0 , aParam2.priority );
		assertTrue     (aParam2.dependencies().isEmpty());
		assertEquals   (aParam2.content(), VacPropertyToAlloyCode.EMPTY_CONVERTOR.generateAlloyCode());
