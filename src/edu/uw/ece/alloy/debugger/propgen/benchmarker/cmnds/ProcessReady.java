...
public class ProcessReady extends RemoteCommand {
	...
	public void activateMe(ProcessesManager manager) {
		/*
		 * try { (new
		 * RegisterCallback(manager.getProcessRemoteMonitorAddress())).sendMe(
		 * manager.getAlloyProcess(pId).address); } catch (InterruptedException e) {
		 * logger.log(Level.SEVERE, "["+Thread.currentThread().getName()+"] "+
		 * "Failed to transfer registercallback: "+pId); }
		 */
