...
public class ProcessorUtil {
	...
	protected final static Logger logger = Logger
			.getLogger(ProcessorUtil.class.getName() + "--" + Thread.currentThread().getName());;
	...
	public static class EmptySocketFinder {
		...
		public final static int MinPortNumberOffset = Integer.valueOf(Configuration.getProp("min_port_offset"));
		...
		protected EmptySocketFinder(int startingPort, int minPortOffset, int maxPort) {
			this.minPort = startingPort + (startingPort % minPortOffset) * minPortOffset;
		...
		public boolean available(int port) {
			...
			try {
				channel = AsynchronousServerSocketChannel.open()
						.bind(new InetSocketAddress(InetAddress.getLocalHost().getHostAddress(), port));
				return true;
			} catch (IOException e) {
				...
				if (channel != null) {
					try {
						...
						logger.severe(
								Utils.threadName() + "Open port '" + port + "' cannot be closed: " + e.getMessage());
					}
				}
			}
		 ...
		 * @return
		 */
		public synchronized InetSocketAddress findEmptyLocalSocket(final InetAddress localAddress) {
			...
			while (++findPortTriesMax < MaxTryPort) {
				tmpPort = (tmpPort + 1)
						% (maxPort - minPort);/*
												 * The range is an odd number so
												 * the second round it iterates
												 * the other sent of numbers.
												 */
	 ...
	 * @return
	 */
	public static synchronized InetSocketAddress findEmptyLocalSocket(int initialPort) {

		if (null == socketFinder) {
			socketFinder = new EmptySocketFinder(initialPort, EmptySocketFinder.MinPortNumberOffset,
					EmptySocketFinder.MaxPortNumber);
		}

		try {
			return socketFinder
					.findEmptyLocalSocket(InetAddress.getByName(InetAddress.getLocalHost().getHostAddress()));
		} catch (UnknownHostException e) {
			logger.log(Level.SEVERE,
					"[" + Thread.currentThread().getName() + "] " + "The Local IP address cannot be found ", e);
			e.printStackTrace();
	 ...
	 * @throws IOException
	 */
	public static Process createNewJVM(int SubMemory, int SubStack, String ProcessLoggerConfig,
			InetSocketAddress remoteSocket, InetSocketAddress localSocket, Class<?> clazz) throws IOException {

		final String java = "java";
		final String debug = Boolean.parseBoolean(System.getProperty("debug")) ? "yes" : "no";

		try {
			ProcessBuilder pb = new ProcessBuilder(java, "-Xmx" + SubMemory + "m", "-Xss" + SubStack + "k",
					"-Ddebug=" + debug, "-XX:-UseConcMarkSweepGC", "-XX:-UseGCOverheadLimit",
					"-Djava.util.logging.config.file=" + ProcessLoggerConfig, "-cp",
					System.getProperty("java.class.path"), clazz.getName(), "" + remoteSocket.getPort(),
					"" + remoteSocket.getAddress().getHostAddress(), "" + localSocket.getPort(),
			...
			logger.log(Level.SEVERE, "[" + Thread.currentThread().getName() + "]"
					+ "Not able to create a new process on port: " + remoteSocket, e);
			throw e;
		}
	}
