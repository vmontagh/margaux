...
public class AlloyFeeder extends GeneratedStorage<AlloyProcessingParam>
	...
	// the request is
	// added to 'backLog' then merged into 'queue'. The 'backLog' could be either
	// limit or unlimited.
	...
	public AlloyFeeder(final ProcessesManager processesManager,
			final int bufferSize, final int backLogBufferSize) {
		super();
	...
	public void addProcessTask(final AlloyProcessingParam p)
			throws InterruptedException {
		if (Configuration.IsInDeubbungMode)
			logger.log(Level.INFO, "[" + Thread.currentThread().getName() + "]"
					+ " a request is added to be sent:" + p);
		...
		try {
			...
			logger.log(Level.SEVERE,
					"[" + Thread.currentThread().getName() + "]"
							+ "a new Alloy process message cannot be added to the queue:" + p,
					e);
			throw e;
		}
		if (Configuration.IsInDeubbungMode)
	...
	public void addGeneratedProp(final AlloyProcessingParam item) {
		if (!PREVENT_DUPLICATION || !(monitor.isDone(item) || monitor.isTobeDone(item)))
			try {
				...
				logger.log(Level.SEVERE, "[" + Thread.currentThread().getName() + "]"
						+ "Cannot add a new Alloy procesing param:" + item, e);
				e.printStackTrace();
				throw new RuntimeException(e);
			}
	}
	...
	public void addProcessTaskToBacklog(final AlloyProcessingParam p)
			throws InterruptedException {
		if (Configuration.IsInDeubbungMode)
			logger.log(Level.INFO, "[" + Thread.currentThread().getName() + "]"
					+ " a request is added to be merged:" + p);
		try {
			backLog.put(p);
		} catch (InterruptedException e) {
			logger.log(Level.SEVERE,
	...
	private void merge() throws InterruptedException {
		...
		if (Configuration.IsInDeubbungMode)
			logger.info("[" + Thread.currentThread().getName() + "]"
					+ "Backlog Queue size is: " + backLog.size());
		AlloyProcessingParam e;
		try {
			// take a request, if something is in the queue. Otherwise the thread
			// parks here.
		...
		if (Configuration.IsInDeubbungMode)
			logger.info("[" + Thread.currentThread().getName() + "]"
					+ "Message is taken " + e + " and backLog size is:" + backLog.size());
		...
		if (Configuration.IsInDeubbungMode)
			logger.info("[" + Thread.currentThread().getName() + "]"
					...
					+ " and the backlogqueue size is: " + backLog.size());
	...
	public String getStatus() {
		return (new StringBuilder()).append("New messages added=").append(size)
