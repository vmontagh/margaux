...
public class HolaCompleteCommand extends RemoteCommand {
	...
	public HolaCompleteCommand(final InetSocketAddress pID, final HolaResult result) {
		super();
		PID = pID;
		this.result = result;
	}
	...
	public void send(final InetSocketAddress remoteAddres) throws InterruptedException{
		...
		if(Configuration.IsInDeubbungMode) logger.fine("["+Thread.currentThread().getName()+"] " + "Sending a response: pID= "+PID +" result="+result.toString());
		
		try {
			
			this.sendMe(remoteAddres);
			
			if(Configuration.IsInDeubbungMode) logger.fine("["+Thread.currentThread().getName()+"] " + "Response is sent: pID= "+PID +" result="+result.toString());
