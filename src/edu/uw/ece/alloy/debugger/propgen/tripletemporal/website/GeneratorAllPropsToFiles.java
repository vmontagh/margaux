...
public class GeneratorAllPropsToFiles {
	...
	public GeneratorAllPropsToFiles() {
		...
		dependencies.add(Dependency.EMPTY_DEPENDENCY.createIt(new File(relationalPropModuleOriginal.getName())));

		propertyBuilder = new PropertyToAlloyCodeBuilder(dependencies,
				/* TemporalPropertiesGenerator.Header */ "open relational_properties\n",
				""/*
					 * , AlloyProcessingParam.EMPTY_PARAM.changeTmpLocalDirectory(
					 * outputDir)
					 */// [tmpDirectory],tmpDirectory
		);

		propertyBuilder.registerPropertyToAlloyCode(SinglePropertyToAlloyCode.EMPTY_CONVERTOR);
	}

	public void genPropsAsHTMLControls() {

		final String select = "<label for='__N_A_M_E__'>__N_A_M_E__:</label>\n"
				+ "<select name='__N_A_M_E__' id='__N_A_M_E__'>\n" + "__O_P_T_I_O_N_S__" + "</select>";
		...
		for (final String feature : features.keySet()) {
			...
			result += select.replaceAll("__N_A_M_E__", feature).replaceAll("__O_P_T_I_O_N_S__", options);
		}
	...
	public void genPropsBodyAasHTML() {
		...
		for (String pred : props.keySet()) {
			for (final PropertyToAlloyCode alloyCodeGenerator : propertyBuilder.createObjects(props.get(pred).b,
					Compressor.EMPTY_STRING, props.get(pred).a, Compressor.EMPTY_STRING, pred, Compressor.EMPTY_STRING// [tmpDirectory],
																														// tmpDirectory
					, Compressor.EMPTY_STRING)) {

				try {
					final AlloyProcessingParam generatedParam = new AlloyProcessingParam(UUID.randomUUID(), 0,
							alloyCodeGenerator);
