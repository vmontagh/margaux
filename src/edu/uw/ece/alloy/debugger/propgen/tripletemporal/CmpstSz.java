...
public abstract class CmpstSz extends Property {
	...
	public CmpstSz(String rName, String sName, String sNext, String sFirst,
	...
	protected boolean isConsistent() {
		...
		if (!size1.RName.equals(size2.RName))
			return false;
		if (!size1.SName.equals(size2.SName))
			return false;
		if (!size1.SNext.equals(size2.SNext))
			return false;
		if (!size1.SFirst.equals(size2.SFirst))
			return false;
		if (!size1.MiddleName.equals(size2.MiddleName))
			return false;
		if (!size1.EndName.equals(size2.EndName))
			return false;
		if (!size1.RConcreteName.equals(size2.RConcreteName))
			return false;
		if (!size1.SConcreteName.equals(size2.SConcreteName))
			return false;
		if (!size1.SConcreteNext.equals(size2.SConcreteNext))
			return false;
		if (!size1.SConcreteFirst.equals(size2.SConcreteFirst))
			return false;
		if (!size1.MConcreteName.equals(size2.MConcreteName))
			return false;
		if (!size1.EConcreteName.equals(size2.EConcreteName))
			return false;

		if (!size1.growthLocality.equals(size2.growthLocality))
			return false;
		if (!size1.empty.equals(size2.empty))
			return false;

		if (!size1.isConsistent())
			return false;
	...
	public String genPredName() {
		return super.genPredName() + size1.genPredName() + "_"
				+ size2.genPredName();
	}
