...
public class SpecHasPropertyChecking extends PropertyCheckingSource {
	...
	final String predicateANDNameFormat = new String(
			"%s" + SEPARATOR + "_A_n_D_" + SEPARATOR + "%s" + SEPARATOR + "%s");
	...
	public SpecHasPropertyChecking(File sourceFile_, String property_,
		...
		this.predicateName = String.format(predicateANDNameFormat, commandHeader,
				sanitizedPropertyName, sanitizedFieldName);
		String empty = "";
		if (!property_.contains("EMPTY"))
			empty = " and " + emptyProperty;
		this.predicateBody = String.format(propertyRunningFormat, predicateName,
				property, formula, commandScope, empty);
	}
	...
	public boolean repOk() {
		return super.repOk() && this.predicateName != null
				&& !this.predicateName.trim().isEmpty() && this.predicateBody != null
				&& !this.predicateBody.trim().isEmpty();
	}
