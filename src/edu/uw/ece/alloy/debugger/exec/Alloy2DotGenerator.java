...
public class Alloy2DotGenerator {
	...
	/*
	 * public static int PACE = 1; final public static boolean oneFound = false;
	 * final public static boolean usingKodkod = true; final public static boolean
	 * usingKKItr = false; final public static boolean symmetryOff = false; final
	 * public static boolean usingSymmetry = true; public static String
	 * reportFileName = "";
	 */
	...
	public static class QuickReporter extends A4Reporter {
		...
		public void solve(final int primaryVars, final int totalVars,
				final int clauses) {
			System.out.println("solve->" + totalVars + " vars. " + primaryVars
					+ " primary vars. " + clauses + " clauses. "
		...
		public void translate(String solver, int bitwidth, int maxseq,
			...
			lastTime = System.currentTimeMillis();
			System.out.println(
					"translate->Solver=" + solver + " Bitwidth=" + bitwidth + " MaxSeq="
							+ maxseq + (skolemDepth == 0 ? "" : " SkolemDepth=" + skolemDepth)
	...
	public static String run(String[] args, A4Reporter rep) throws Err {
		...
		for (String filename : args) {
			...
			for (Command command : world.getAllCommands()) {
				...
				System.out.printf(
						"The command is: %s %n and the formunla is: %s %n%n%n",
						command.label, command.formula);

				String dotContent = MakeDOTCallGraph.makeString(command.label,
						command.formula);
				// Where the output file goes? The output file is a dot file and is
				// determined from he input. By default
				// it will be located beside the input file.
				String dotoutput = filename.replace(".als",
						"." + command.label + ".dot");

				Util.writeAll(dotoutput,
						String.format("digraph G {%n %s %n}", dotContent));
	...
	private static Command excludeCommandScope(final Command command,
			final Set<String> excludedSigs) {
	...
	/*
	 * private static UniqSigMessage makeWorld(final Command command, final
	 * CompModule world, final Sig s, final A4Options options, final A4Reporter
	 * rep, final UniqSigMessage usm) throws Err{
	 * 
	 * 
	 * 
	 * Bounds oBound = new Bounds(usm.oBound); List<CommandScope> nList = new
	 * ArrayList<CommandScope>(usm.nList);
	 * 
	 * CommandScope sigScope = usm.sigScope==null? null :
	 * (CommandScope)usm.sigScope.clone(); CommandScope scope = usm.scope == null?
	 * null :(CommandScope)usm.scope.clone();
	 * 
	 * Map<String,ExprVar> sigAtoms = //usm.sigAtoms==null? new
	 * HashMap<String,ExprVar>() //: new HashMap<String,ExprVar>(usm.sigAtoms) ;
	 * 
	 * 
	 ...
	 * 
	 * Pair<A4Solution,List<Instance>> legalPair =
	 * (Pair<A4Solution,List<Instance>>)TranslateAlloyToKodkod.
	 * evaluate_command_Itreational( rep, world.getAllReachableSigs(), command,
	 * options,s,expr);
	 * 
	 * 
	 * for(Instance i: legalPair.b){ LoggerUtil.Detaileddebug(
	 * "The instance is: %s", i); }
	 * 
	 * System.exit(-10);
	 * 
	 ...
	 * 
	 * 
	 * Set<String> fldNames = new HashSet<String>(); for(Decl
	 * fDecl:s.getFieldDecls()){ A4TupleSet legal =
	 * legalPair.a.getfieldSolutions(legalPair.b, s.label+"."+fDecl.get().label);
	 * fldNames.add(s.label+"."+fDecl.get().label);
	 * 
	 * List<List<Expr>> pFields = new ArrayList<List<Expr>>();
	 * 
	 * 
	 * //Make a list of tuples for the field and a set of atoms for the left-most
	 * sig List<Expr> field; for(A4Tuple tuple: (A4TupleSet)legal){ field = new
	 * ArrayList<Expr>(); for(int i=0; i<tuple.arity(); i++){ if(i==0 &&
	 * s.isOne==null) sigAtoms.put(tuple.atom(i), ExprVar.make(command.bound.pos,
	 * tuple.atom(i))) ; field.add(ExprVar.make(command.bound.pos, tuple.atom(i))
	 * ); } pFields.add(field);
	 * 
	 * } //Merge the CommandScope //First find the related commandscope if it
	 * exists. List<CommandScope> oScopes = oBound.scope; nList.clear(); scope =
	 * null; for(CommandScope sc: oScopes){
	 * if(sc.sig.label.equals(fDecl.get().label)){ scope = sc; }else
	 * if(sc.sig.label.equals(s.decl.get().label)){ sigScope = sc; }else{
	 * nList.add(sc); } }
	 * 
	 * 
	 * if( scope==null){ //Make a new commandscope for each relation declration
	 * scope = new CommandScope( oBound.pos, new PrimSig(fDecl.get().label,
	 * AttrType.WHERE.make( oBound.pos)), true, pFields.size(), pFields.size(), 1,
	 * new ArrayList<ExprVar>(), pFields.size(), pFields, true, false,
	 * false,false); }else{ //Alter the current commandscope pFields.addAll(
	 * scope.pFields); scope = new CommandScope( scope.pos, scope.sig,
	 * scope.isExact, pFields.size(), pFields.size(), scope.increment,
	 * scope.pAtoms, pFields.size()+ scope.pAtomsLowerLastIndex, pFields,
	 * scope.isPartial, scope.hasLower, scope.hasUpper, scope.isSparse); }
	 * 
	 * 
	 ...
	 * 
	 * 
	 * Bounds nBound = new Bounds( oBound.pos, oBound.label, nList); oBound =
	 * world.replaceBound( oBound, nBound);
	 * 
	 * 
	 * }//End of For
	 * 
	 * 
	 * 
	 * //Add the an empty instance to the solution if it is valid. Object hasEmpty
	 * = legalPair.a.getEmpty(legalPair.b, fldNames, s);
	 * 
	 * 
	 * 
	 * 
	 * if(hasEmpty!=null ){ sigAtoms.put(hasEmpty.toString(),
	 * ExprVar.make(command.bound.pos, hasEmpty.toString(),s.type())); }
	 * 
	 * LoggerUtil.debug(ExampleUsingTheCompiler.class,
	 * "sigScope in makeworld->%s%n scope->%s%n oBound->%s%n nList->%s%n sigAtoms->%s%n"
	 * ,sigScope,scope,oBound,nList,sigAtoms);
	 * 
	 * 
	...
	private static Expr getEmptyCheckingExpr(Sig sig) {
		...
		ExprHasName uSigName = ExprVar.make(null,
				sig.label + Math.abs(sig.label.hashCode()), sig.type());
		...
		for (Decl fDecl : sig.getFieldDecls()) {
			...
			for (int i = 2; i < fld.type().arity(); i++) {
				empty = ExprBinary.Op.ARROW.make(null, null, empty,
			...
			if (epmtyChkrExpr == null) {
				...
				epmtyChkrExpr = ExprBinary.Op.AND.make(null, null, epmtyChkrExpr,
						emptyExpr);
			}
		}
		if (epmtyChkrExpr != null) {
