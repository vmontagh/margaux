...
public class RelationalPropertiesRelationChecker {
	...
	static void run (final String _s, final String sameType) throws IOException, Err, InterruptedException
	{
		...
		final File dummyFile = new File(tmpDirectory, "dummy_"+_s+"_"+sameType+".als");
		final String dummyAlloy = sameType.toUpperCase().equals("SAMETYPE") ? "sig A{r:A->A}\nrun {}" :  "sig A,B{}\nsig C{r:A->B}\nrun {}";
		edu.mit.csail.sdg.alloy4.Util.writeAll(dummyFile.getAbsolutePath(), dummyAlloy);
		...
		try {
			if(_s.equals("and")){
				propCheckingFiles = propertiesChecker.findInconsistentProperties(tmpDirectory);
			} 
			if(_s.equals("imply")){
				propCheckingFiles = propertiesChecker.findImplicationsProperties(tmpDirectory);
			}
			assert propCheckingFiles != null; 
			System.out.printf("%d files are enerated to be checked.%n",propCheckingFiles.size());
			BenchmarkRunner.getInstance().doTest(new RelationalPropertiesExecuterJob(""), 1, 1, propCheckingFiles, logOutput);
