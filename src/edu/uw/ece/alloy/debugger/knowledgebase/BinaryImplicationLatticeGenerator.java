...
/**
 * Using PropertyCallBuilder and PropertyCheckingBuilder
 * This class makes an Alloy file showing the implication
 * relations.
 * @author vajih
 *
 */
public class BinaryImplicationLatticeGenerator {
	...
	public static void main(String...args) throws Err{
		...
		String tggedLibrary = Utils.readFile(relationalPropModuleOriginal.getAbsolutePath());
		tggedLibrary += "\n";
		tggedLibrary += relationDefinition;
		
		// Store back the merged file into the temp directory.
		File tmpFile = new File(tmpDirectoryRoot, "relational_properties_tagged.als");
		Util.writeAll(tmpFile.getAbsolutePath(), tggedLibrary);
		
		CompModule world = 
				CompUtil.parseEverything_fromFile(A4Reporter.NOP, null,
		...
		/*for (String p1: pcb.makeAllBinaryProperties(fields.get(1))){
			for (String p2: pcb.makeAllBinaryProperties(fields.get(1))){
				if (p1.equals(p2)) continue;
					String p1Name = p1.substring(0, p1.indexOf("[") );
					String p2Name = p2.substring(0, p2.indexOf("[") );
