...
import edu.uw.ece.alloy.graph.AdjMatrixEdgeWeightedDigraph;
import edu.uw.ece.alloy.graph.DirectedEdge;
import edu.uw.ece.alloy.graph.FloydWarshall;
import edu.uw.ece.alloy.util.Utils;

/**
 * Given the implication info, the class generates an Alloy
 * file to infer the implications.
 * The given info,has to be two csv files:
 * 1- Implication relation
 * 2- legend relation
 * @author vajih
 *
 */
public class TemporalImplicationLatticeGenerator {
	...
	public TemporalImplicationLatticeGenerator(String pathToLegend,
		...
		for (String line: Utils.readFileLines(pathToIff)){
			...
			iffMap.get(legends.get(splittedRow[0])).add(
					legends.get(splittedRow[1]));
		}
		...
		for (String line: Utils.readFileLines(pathToImplication)){
			...
			assert legends.containsKey(splittedRow[0]);
			assert legends.containsKey(splittedRow[1]);
			assert implicationMap.containsKey(groupingMap.get(legends.get(splittedRow[0])));
			implicationMap.get(					
					groupingMap.get(legends.get(splittedRow[0]))).add(
							groupingMap.get(legends.get(splittedRow[1])));
		}
	...
	protected Map<String, List< String>> findAllReachables(Map<String, Set< String>> input){
		Map<String, List<String>> result = new HashMap<>();
	...
	/**
	 * Find all reachable properties from a property
	 * E.g., a=>b, b=>c. return [a->{b,c}]
	 * @return
	 */
	...
	public static void main(String...args){
		TemporalImplicationLatticeGenerator generator = 
				new TemporalImplicationLatticeGenerator(pathToLegend,
						pathToImplication,
						pathToIff);
		...
		for (String key: generator.groupingMap.keySet()){
			...
			if (wasTheSame)
				++notGrouped;
			System.out.println("Self?"+(wasTheSame)+"\t"+key+"-->"+generator.groupingMap.get(key));
		}
