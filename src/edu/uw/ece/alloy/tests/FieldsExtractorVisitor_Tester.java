...
public class FieldsExtractorVisitor_Tester {
	...
	public void test_FieldsExtractorCommand1() throws Err {
		...
		Module world = CompUtil.parseEverything_fromFile(null, null,
				testingAlloyFile);

		assertEquals(FieldsExtractorVisitor
				.getReferencedFields(world.getAllCommands().get(0).formula).stream()
				.sorted((Sig.Field a, Sig.Field b) -> {
					return a.toString().compareTo(b.toString());
				}).collect(Collectors.toList()).toString(),
				"[field (this/B <: s), field (this/C <: t)]");
	...
	public void test_FieldsExtractorCommand2() throws Err {
		...
		System.out.println(FieldsExtractorVisitor
				.getReferencedFields(world.getAllCommands().get(1).formula).toString());

		assertEquals(FieldsExtractorVisitor
				.getReferencedFields(world.getAllCommands().get(1).formula).stream()
				.sorted((Sig.Field a, Sig.Field b) -> {
					return a.toString().compareTo(b.toString());
				}).collect(Collectors.toList()).toString(),
				"[field (this/B <: r), field (this/C <: t)]");
